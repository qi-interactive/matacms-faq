...
<div class="list-container row <?= empty($model->filterableAttributes()) ? 'simple-list-container' : ''; ?>">
	<?php if ($uri = $model->getVisualRepresentation()) {
		...
		<a href='<?= sprintf("%s/update?SubjectId=%d&id=%d", $moduleBaseUrl, $model->SubjectId, $model->primaryKey );?>' class="list-link">
		...
		<a class='delete-btn' href="<?= sprintf("%s/delete?SubjectId=%d&id=%d", $moduleBaseUrl, $model->SubjectId, $model->primaryKey );?>" <?php if(method_exists($model, 'canBeDeleted')) {
